...
with tab3:
    if st.session_state.roast_status == "completed" and st.session_state.roast_results:
        ...
        # Add a download button for the entire roast (JSON format)
        st.download_button(
            label="Download Roast Results (JSON)",
            data=json.dumps(results, indent=4),
            file_name="roast_results.json",
            mime="application/json",
            key="download-roast-json",
            help="Download the complete roast results as a JSON file."
        )
        ...
        if "summary" in results:
            ...
            # Copy summary button using Streamlit
            if st.button("Copy Summary", key="copy_summary"):
                try:                  
                    st.session_state.clipboard_content = results["summary"]
                    st.success("Summary copied to clipboard! Use Ctrl+V to paste.")
                except Exception as e:
        ...
        roasted_files = results.get("roasted_files", [])
        if roasted_files:
            for file_data in roasted_files:
                # Each file_data is typically [file_path, critique_text, suggestions_text?]
                if len(file_data) >= 2:
                    ...
                    with st.expander(f"📄 {file_path}", expanded=True):
                        ...
                        # Copy button for critique
                        critique_js = json.dumps(critique)
                        copy_critique_button = f"""
                        <button class="copy-button" onclick="navigator.clipboard.writeText({critique_js})">
                            Copy Critique
                        </button>
                        """
                        st.markdown(copy_critique_button, unsafe_allow_html=True)
                        
                        # Suggestions section
                        if len(file_data) >= 3:
                            suggestions_text = file_data[2]
                            if suggestions_text:
                                suggestions_html = f"""
